...
class infer(_Jac.Walker):
    ...
    def init_router(self, _jac_here_: _Jac.RootType) -> None:
        ...
        if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, Router)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
            ...
            _Jac.connect(left=router_node, right=FDChat(), edge_spec=_Jac.build_edge(is_undirected=False, conn_type=None, conn_assign=None))
    ...
    def route(self, _jac_here_: Router) -> None:
        ...
        if classification == ChatType.RAG:
            ...
            if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, RagChat)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
                pass
        elif classification == ChatType.QA:
            print('Routing to QAChat')
            if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, QAChat)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
                pass
        elif classification == ChatType.FEEDBACK:
            print('Routing to FeedbackChat')
            if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, FDChat)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
                pass
        else:
